...
class User_Model_Mailer
{
     ...
     * @since 0.2
     */
    final public static function sendRegistrationConfirmation(User_Model_User $user)
    {
        $mail = new Zend_Mail();
     ...
     * @since 0.2
     */
    final public static function sendUsername(User_Model_User $user)
    {
        ...
        $mail->setBodyText(
            "Thank you for registering at  " . Zend_Registry::get('config')->site->name . " To login to your new account, we just need you to verify your email address and activate your account,
		which can be done my clicking on the link below:
    ...
    final public static function sendPassword(User_Model_User $user)
    {
        //Create an instance of Zend Mail and set the header info
        $mail = new Zend_Mail();
        $mail->addTo($user->getEmail(), $user->getFname() . " " . $user->getLname());
        $mail->setFrom(Zend_Registry::get('config')->email->noreply, Zend_Registry::get('config')->email->name);
        $mail->setSubject('Your reset password');

        // Create the HTML version of the message body
        $mail->setBodyHtml(
            "<p>You are receiving this email becausesomeone claiming to be the account owner has requested
        to have the account password reset.  If this request was not made by you please immediately login to your account
        ...
        // Create a Text version of the message body
        $mail->setBodyText(
            "You are receiving this email becausesomeone claiming to be the account owner has requested
        to have the account password reset.  If this request was not made by you please immediately login to your account
        and change your password.
        We recommend that you change this password immediately after logging in.
        Password: " . $user->getPassword() . "
        This is an automated message.  Please Do Not Reply to this email.  If you require assistance, please contact
